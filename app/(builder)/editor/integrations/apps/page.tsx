...
export default function AppsPage() {
  ...
  const fetchApps = async () => {
    try {
      const response = await fetch(projectsPath('/api/apps'));
  ...
  const loadMailerLiteSettings = async () => {
    ...
    try {
      const response = await fetch(projectsPath('/api/apps/mailerlite/settings'));
      const result = await response.json();
  ...
  const handleTestApiKey = async () => {
    ...
    setIsTesting(true);
    try {
      const response = await fetch(projectsPath('/api/apps/mailerlite/test'), {
        method: 'POST',
  ...
  const handleDisconnect = async () => {
    try {
      await fetch(projectsPath('/api/apps/mailerlite/settings'), {
        method: 'DELETE',
      });
  ...
  const loadTokenApp = async (appId: string) => {
    ...
    try {
      const response = await fetch(projectsPath(`/api/apps/${appId}/settings`));
      const result = await response.json();
  ...
  const handleDisconnectTokenApp = async (appId: string) => {
    ...
    try {
      await fetch(projectsPath(`/api/apps/${appId}/settings`), { method: 'DELETE' });
      updateTokenAppState(appId, {
        token: '',
        savedToken: '',
        isConnected: false,
  ...
  const loadGroupsAndForms = useCallback(async () => {
    ...
    try {
      const [groupsRes, formsRes] = await Promise.all([
        fetch(projectsPath('/api/apps/mailerlite/groups')),
  ...
  const renderConnectionForm = () => (
    <>
      {/* Form Selection */}
      <Field>
        <FieldLabel>Avi Form</FieldLabel>
        {isLoadingForms ? (
