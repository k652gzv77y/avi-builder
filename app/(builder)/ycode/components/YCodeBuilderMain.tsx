...
export default function YCodeBuilder({ children }: YCodeBuilderProps = {} as YCodeBuilderProps) {
  ...
  useEffect(() => {
    const checkSupabaseConfig = async () => {
      try {
        const response = await fetch(projectsPath('/api/setup/status'));
        const data = await response.json();

        if (!data.is_configured) {
          // Redirect to setup wizard
          router.push(projectsPath('/welcome'));
          return;
        }
  ...
  // unauthenticated users through `/projects/:slug` and back). Only same-origin
  // paths starting with `/projects/:slug` are accepted to prevent open redirects.
  useEffect(() => {
    if (!user || !authInitialized) return;
    const next = searchParams?.get('next');
    if (!next) return;
    if (!next.startsWith(projectsPath(''))) return;
    router.replace(next);
        ...
        <div className="w-full max-w-sm animate-in fade-in slide-in-from-bottom-1 duration-700" style={{ animationFillMode: 'both' }}>
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-3">
              <button
                type="button"
                className="flex h-10 w-full items-center justify-center gap-2 rounded-full border border-white/25 bg-transparent px-4 text-sm font-medium text-white transition-colors hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white disabled:cursor-not-allowed disabled:opacity-60"
          ...
          <div className="mt-4 text-center">
            <p className="text-xs text-white/50">
              First time here?{' '}
              <Link href={projectsPath('/welcome')} className="text-white/80">
                Complete setup
              </Link>
            </p>
          </div>
