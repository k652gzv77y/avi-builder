...
export const usePagesStore = create<PagesStore>((set, get) => ({
  ...
  publishPage: async (pageId) => {
    set({ isLoading: true, error: null });
    try {
      const response = await fetch(projectsPath('/api/publish'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pageIds: [pageId] }),
      });
  ...
  duplicatePage: async (pageId) => {
    ...
    (async () => {
      try {
        const response = await fetch(projectsPath(`/api/pages/${pageId}/duplicate`), {
          method: 'POST',
        });
        ...
        try {
          const [pagesResponse, foldersResponse] = await Promise.all([
            fetch(projectsPath('/api/pages')),
            fetch(projectsPath('/api/folders'))
          ]);
  ...
  duplicateFolder: async (folderId) => {
    ...
    (async () => {
      try {
        const response = await fetch(projectsPath(`/api/folders/${folderId}/duplicate`), {
          method: 'POST',
        });
  ...
  refetchPageCollectionItem: async (pageId) => {
    ...
    try {
      ...
      const response = await fetch(projectsPath(`/api/pages/${pageId}/collection-item?itemId=${currentItem.id}`));
      const result = await response.json();
